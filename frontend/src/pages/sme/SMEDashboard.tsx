    ...
    try {
      ...
      if (jobsData && jobsData.length > 0) {
        ...
        if (bidsError) {
          ...
          const studentIds = [...new Set(bidsData.map((bid) => bid.student_id))];
          ...
          const bidsWithProfiles = bidsData.map((bid) => {
            ...
            return {
              ...bid,
              profiles: profileMap[bid.student_id] || { full_name: "Student", email: "" },
              jobs: job,
            };
          });
  ...
  const handleAcceptBid = async (bidId: string, jobId: string, studentId: string, jobTitle: string) => {
    ...
    try {
      ...
      await supabase.from("notifications").insert({
        ...
        type: "bid_accepted",
        title: "Bid Accepted! 🎉",
        message: `Your bid for "${jobTitle}" has been accepted. You can now chat with the client.`,
        related_id: jobId,
        is_read: false,
      });
                      ...
                      <div className="job-card-footer">
                        <div>
                          ...
                          {job.status === "in_progress" && (
                            <span style={{ display: "block", fontSize: "0.7rem", color: "#856404", marginTop: "0.25rem" }}>
                              <FiClock size={10} /> In Progress - Student Hired
                            </span>
                          )}
                        </div>
                        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                          <button className="btn-primary btn-small" onClick={() => viewJobBids(job)} style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                            <FiEye /> View Bids ({getBidCount(job.id)})
                          </button>
                          {acceptedBid && (
                            <button className="btn-outline btn-small" onClick={() => goToChat(acceptedBid.student_id, acceptedBid.profiles?.full_name)} style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                              <FiMessageCircle size={12} /> Message Student
                            </button>
                          )}
                        </div>
                      </div>
        ...
        return (
          <div>
            <div className="section-header">
              ...
              <p className="topbar-sub">Find verified students to hire for your projects</p>
            </div>
                ...
                {students.map((student) => (
                  <div className="bidder-card" key={student.id}>
                    <div className="bidder-header">
                      <div className="bidder-avatar">{student.full_name?.charAt(0).toUpperCase() || "S"}</div>
                      <div className="bidder-details">
                        <p className="bidder-name">{student.full_name || "Student"}</p>
                        <span className="bidder-job">{student.skills?.[0] || "Student"}</span>
                      </div>
                    </div>
                    <div className="bidder-stats">
                      <div className="bidder-rating">
                        <FiStar className="star" />
                        <span className="rating-score">{student.rating || "4.5"}</span>
                        <span className="review-count">({student.total_jobs || 0} jobs)</span>
                      </div>
                      <div className="bidder-completed">
                        <FiCheckCircle size={12} />
                        <span>{student.matric_number ? "✓ Verified" : "Student"}</span>
                      </div>
                    </div>
                    <div className="bidder-footer">
                      <div className="bid-price">
                        <span className="price-label">{student.bio?.substring(0, 50) || "Available for work"}</span>
                      </div>
                      <div className="bidder-actions">
                        <button className="btn-primary btn-small" onClick={() => {
                          const openJob = jobs.find((j) => j.status === "open");
                          if (openJob) {
                            alert(`Invite ${student.full_name} to job: ${openJob.title}`);
                          } else {
                            alert(`Post a job first to invite ${student.full_name}`);
                          }
                        }}>Hire</button>
                        <button className="btn-outline btn-small" onClick={() => goToChat(student.id, student.full_name)}>
        ...
        return (
          <div className="payments-page">
            ...
            <div className="stats-grid">
              <div className="stat-card stat-green">
                ...
                <div className="stat-info">
                  <p className="stat-value">{new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN" }).format(totalSpent)}</p>
                  <p className="stat-label">Total Spent</p>
                </div>
              </div>
              <div className="stat-card stat-blue">
                <div className="stat-card-icon"><FiLock /></div>
                <div className="stat-info">
                  <p className="stat-value">{new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN" }).format(inEscrow)}</p>
              ...
              <div className="transactions-list">
                {transactions.map((tx) => (
                  <div className="transaction-item" key={tx.id}>
                    <div className={`tx-icon ${tx.status === "released" ? "tx-out" : "tx-hold"}`}>
                      {tx.status === "released" ? <FiCheckCircle /> : <FiLock />}
                    </div>
                    <div className="tx-info">
                      <p className="tx-title">{tx.jobs?.title || "Job Payment"}</p>
                      <p className="tx-sub">
                        {tx.status === "held" ? "In Escrow" : tx.status === "released" ? "Released" : "Pending"} •
                        {new Date(tx.created_at).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="tx-right">
                      <span className="tx-amount">{new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN" }).format(tx.amount)}</span>
                      <span className="tx-status" style={{ color: tx.status === "released" ? "#1a9c6e" : "#f59e0b" }}>
                        {tx.status === "held" ? "Held" : tx.status === "released" ? "Completed" : "Pending"}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <div className="escrow-info-card">
              <div className="escrow-info-icon"><FiLock /></div>
              <div>
                <h3>How Escrow Works</h3>
                <p>Payments are held securely until you confirm job completion, then released to the student.</p>
              </div>
            </div>
          </div>
        );
            ...
            <div className="profile-card">
              <div className="profile-photo-wrap">
                <div className="profile-photo">
                  <div className="profile-photo-placeholder">{userName?.charAt(0).toUpperCase()}</div>
                </div>
              </div>
            ...
            <div className="profile-form-card">
              ...
              <div className="profile-form">
                ...
                <div className="form-group">
                  ...
                  <textarea rows={3} placeholder="Tell students about your business..." />
                </div>
                ...
                {bids.slice(0, 3).map((bid) => (
                  <div className="bidder-card" key={bid.id}>
                    ...
                    <div className="bidder-stats">
                      <div className="bidder-rating">
                        <FiStar className="star" />
                        <span className="rating-score">{bid.amount ? `₦${bid.amount.toLocaleString()}` : "New"}</span>
                      </div>
                      <div className="bidder-completed">
                    ...
                    <div className="bidder-footer">
                      <div className="bid-price">
                        <span className="price-amount">
                          {bid.status === "accepted" ? "✓ Accepted" : bid.status === "rejected" ? "✗ Rejected" : formatBudget(bid.amount)}
                        </span>
                        <span className="price-label">
                          {bid.status === "accepted" ? "Hired" : bid.status === "rejected" ? "Not Selected" : "Bid Amount"}
                        </span>
                      </div>
                      <div className="bidder-actions">
                        <button className="btn-outline btn-small" onClick={() => { setSelectedJob(bid.jobs); setShowBidsModal(true); }}>
                          <FiEye /> View
                        </button>
                        {bid.status === "accepted" && (
                          <button className="btn-primary btn-small" onClick={() => goToChat(bid.student_id, bid.profiles?.full_name)} style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
    ...
    <div className="dashboard">
        ...
        <div className="topbar">
          ...
          <div className="topbar-actions">
            <div style={{ position: "relative" }}>
              ...
              {showNotifications && <NotificationsPopup userId={userId} onClose={() => setShowNotifications(false)} />}
            </div>
            {activePage === "home" && (
      ...
      {showBidsModal && selectedJob && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "white", borderRadius: "20px", width: "90%", maxWidth: "700px", maxHeight: "80vh", overflow: "auto", padding: "1.5rem" }}>
                ...
                getJobBids(selectedJob.id).map((bid) => (
                  <div key={bid.id} style={{ background: bid.status === "accepted" ? "#d4edda" : bid.status === "rejected" ? "#f8d7da" : "white", border: "1px solid #ddd", borderRadius: "12px", padding: "1rem", marginBottom: "1rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", flexWrap: "wrap", gap: "1rem" }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontWeight: "bold", marginBottom: "0.25rem" }}>{bid.profiles?.full_name || "Student"}</p>
                        <p style={{ color: "#666", fontSize: "0.85rem", marginBottom: "0.25rem" }}>{bid.profiles?.email || "No email"}</p>
                        <p style={{ fontSize: "0.9rem", marginBottom: "0.5rem" }}><strong>Bid Amount:</strong> {formatBudget(bid.amount)}</p>
                        <p style={{ fontSize: "0.85rem", marginBottom: "0.5rem" }}><strong>Proposal:</strong> {bid.proposal}</p>
                        <p style={{ fontSize: "0.75rem", color: "#666" }}><strong>Submitted:</strong> {new Date(bid.created_at).toLocaleString()}</p>
                      </div>
                      <div>
                        <span style={{ display: "inline-block", padding: "0.25rem 0.75rem", borderRadius: "20px", fontSize: "0.75rem", fontWeight: "600", background: bid.status === "pending" ? "#fff3cd" : bid.status === "accepted" ? "#d4edda" : "#f8d7da", color: bid.status === "pending" ? "#856404" : bid.status === "accepted" ? "#155724" : "#721c24" }}>
                    ...
                    {bid.status === "pending" && selectedJob.status === "open" && (
                      <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem", paddingTop: "0.5rem", borderTop: "1px solid #ddd" }}>
                        <button onClick={() => handleAcceptBid(bid.id, selectedJob.id, bid.student_id, selectedJob.title)} disabled={processingBid} style={{ padding: "0.5rem 1rem", background: "#1a9c6e", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          <FiCheck /> Accept Bid
                        </button>
                        <button onClick={() => handleRejectBid(bid.id)} disabled={processingBid} style={{ padding: "0.5rem 1rem", background: "#dc2626", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          <FiXCircle /> Reject
                        </button>
                      </div>
                    )}
                    ...
                    {bid.status === "accepted" && (
                      <div>
                        <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem", paddingTop: "0.5rem", borderTop: "1px solid #ddd" }}>
                          <button onClick={() => goToChat(bid.student_id, bid.profiles?.full_name)} style={{ padding: "0.5rem 1rem", background: "#3b82f6", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            <FiMessageCircle /> Message {bid.profiles?.full_name?.split(" ")[0] || "Student"}
                          </button>
                        </div>
                        <PaystackPayment
                          amount={bid.amount}
