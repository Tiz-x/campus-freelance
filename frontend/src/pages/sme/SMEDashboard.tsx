...
const SMEDashboard = () => {
  ...
  const [portfolioFilesMap, setPortfolioFilesMap] = useState<
    Record<string, any[]>
  >({});
  ...
  const fetchPortfolioFiles = async (bidIds: string[]) => {
    ...
    if (!error && data) {
      ...
      data.forEach((file) => {
        if (!filesMap[file.bid_id]) filesMap[file.bid_id] = [];
        filesMap[file.bid_id].push(file);
      });
      setPortfolioFilesMap(filesMap);
    }
  };
  ...
  const fetchBids = async (smeId: string) => {
    try {
      ...
      if (bidsData && bidsData.length > 0) {
        const studentIds = [
          ...new Set(bidsData.map((bid) => bid.student_id).filter(Boolean)),
        ];
        ...
        await fetchPortfolioFiles(bidIds);

        const formattedBids = bidsData.map((bid) => ({
          ...bid,
          profiles: profilesMap[bid.student_id] || {
            full_name: "Student",
            email: "",
            avatar_url: null,
          },
          jobs: jobsMap[bid.job_id] || null,
        }));
                        ...
                        <div className="job-actions">
                          ...
                          {job.status === "in_progress" &&
                            ...
                            acceptedBid && (
                              ...
                              >
                                <FiCheckCircle /> Complete & Release
                              </button>
                            )}
                        </div>
                      </div>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            if (openJob)
                              ...
                              addToast(
                                "Post a job first to invite this student",
                                "warning",
                              );
                          }}
                        >
            ...
            <div className="earnings-hero">
              ...
              <div className="earnings-stats">
                <div className="stat-badge">
                  <span className="stat-value">{formatBudget(inEscrow)}</span>
                    ...
                    <div className="job-day-card" key={bid.id}>
                      ...
                      <div className="job-day-footer">
                        ...
                        <div className="bid-actions">
                          {portfolioFilesMap[bid.id] &&
                            portfolioFilesMap[bid.id].length > 0 && (
                              <div className="portfolio-badge">
                                📎 {portfolioFilesMap[bid.id].length} file(s)
                              </div>
                            )}
                          <button
                            className="apply-btn"
            ...
            <div className="modal-body">
              ...
              {selectedJob.payment_status === "paid" && (
                <p>
                  ...
                  <span style={{ color: "#1a9c6e" }}>✓ Paid - In Escrow</span>
                </p>
              )}
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            setSelectedStudent(bid.profiles);
                            setShowStudentModal(true);
                          }}
                          style={{
                            background: "#6366f1",
                      ...
                      portfolioFilesMap[bid.id].length > 0 && (
                        <div style={{ marginTop: "1rem" }}>
                          <strong>
                            ...
                            {portfolioFilesMap[bid.id].length} files):
                          </strong>
                          <div
                            style={{
                              display: "flex",
                          ...
                          >
                            {portfolioFilesMap[bid.id].map((file, idx) => {
                              const fileName = file.file_url.split("/").pop();
                              const isImage = file.file_url.match(
                                /\.(jpg|jpeg|png|gif)$/i,
                              );
                              return isImage ? (
                                <img
                                ...
                                >
                                  <FiFile /> {fileName?.substring(0, 20)}
                                </a>
                              );
                            })}
                          </div>
  ...
  />
                        ...
                        {selectedJob.payment_status === "paid" && (
                          ...
                          >
                            ✓ Payment of {formatBudget(bid.amount)} is in escrow
                          </div>
                        )}
                      </div>
                    )}
      ...
      {showStudentModal && selectedStudent && (
        <StudentProfileModal
          ...
          onHire={(student) => {
            const openJob = jobs.find(
              (j) => j.status === "open" && j.payment_status !== "paid",
            );
            if (openJob) {
              setSelectedJob(openJob);
              setShowBidsModal(true);
              addToast(
                `You can review bids for "${openJob.title}" to hire ${student.full_name}`,
                "info",
              );
            } else {
              addToast("Post a job first to hire this student", "warning");
            }
          }}
        />
      )}
      ...
      <ConfirmationModal
        ...
        confirmText={
          pendingAction?.type === "logout" ? "Logout" : "Complete & Release"
        }
        cancelText="Cancel"
        type={pendingAction?.type === "logout" ? "warning" : "danger"}
        onConfirm={
          pendingAction?.type === "logout" ? confirmLogout : confirmCompleteJob
        }
        onCancel={() => {
          setShowConfirmModal(false);
          setPendingAction(null);
