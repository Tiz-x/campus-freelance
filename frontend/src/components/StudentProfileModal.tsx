import { FiX, FiStar, FiCheckCircle, FiHash, FiMessageCircle, FiBriefcase,  FiAward,  FiFile, FiGrid, FiChevronLeft, FiChevronRight } from 'react-icons/fi'
        ...
        <div style={{
            ...
            <div style={{ position: 'absolute', bottom: '1.25rem', left: '1.25rem', right: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.8)' }}>
                  {student.department || student.skills?.[0] || 'Student'} · Adeleke University
                </span>
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginBottom: '1.25rem' }}>
              {[
                { label: 'Department', value: student.department?.split(' ')[0] || 'Student', color: '#1a9c6e' },
                { label: 'Faculty', value: student.faculty?.split(' ')[0] || 'Faculty', color: '#8b5cf6' },
                { label: 'Level', value: student.level || '300 Level', color: '#f59e0b' },
              ].map((s, i) => (
                <div key={i} style={{ background: '#f4f6f9', borderRadius: '12px', padding: '1rem', textAlign: 'center', border: '1px solid #e2e8f0' }}>
                  <div style={{ fontSize: '1rem', fontWeight: 800, color: s.color, lineHeight: 1.3 }}>{s.value}</div>
            ...
            <div style={{ marginBottom: '1.25rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {student.department && (
                <div style={{ background: '#f4f6f9', borderRadius: '10px', padding: '0.85rem 1rem', border: '1px solid #e2e8f0' }}>
                  <p style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>Department</p>
                  <p style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0d1b2a', margin: '0.25rem 0 0' }}>{student.department}</p>
                </div>
              )}
              {student.faculty && (
                <div style={{ background: '#f4f6f9', borderRadius: '10px', padding: '0.85rem 1rem', border: '1px solid #e2e8f0' }}>
                  <p style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>Faculty</p>
                  <p style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0d1b2a', margin: '0.25rem 0 0' }}>{student.faculty}</p>
                </div>
              )}
            </div>
            ...
            {student.matric_number && (
              <div style={{ marginBottom: '1.25rem', background: '#f4f6f9', borderRadius: '10px', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#1a9c6e', fontSize: '0.72rem', fontWeight: 700 }}>
                  <FiAward size={14} /> AAUA
                </div>
              </div>
            )}

            {/* PORTFOLIO SECTION - UPDATED WITH GALLERY */}
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {portfolioItems.length > 0 ? (
                ...
                <div style={{ background: '#f4f6f9', borderRadius: '12px', padding: '2rem', textAlign: 'center', border: '1.5px dashed #e2e8f0' }}>
                  ...
                  <p style={{ fontSize: '0.72rem', color: '#cbd5e1', margin: '0.3rem 0 0' }}>Student can add work samples from their profile</p>
                </div>
              )}
            </div>
